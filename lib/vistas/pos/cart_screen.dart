                  ...
                  Expanded(
                    child: isDesktop
                        ? GridView.builder(
                            ...
                            itemBuilder: (context, index) => _buildCartItem(items[index], isMobile: false),
                          )
                        : ListView.separated(
                            padding: const EdgeInsets.all(16),
                            itemCount: items.length,
                            separatorBuilder: (_, __) => const SizedBox(height: 8),
                            itemBuilder: (context, index) => _buildCartItem(items[index], isMobile: isMobile),
                          ),
                  ),
                  ...
                  Row(
                    children: [
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              ...
                              style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF9AC53B)),
                            ),
                          ],
                        ),
                      ),
                      Row(
                        ...
                        children: [
                          IconButton(
                            onPressed: () => widget.onQuantityChanged(item.product.id, -1),
            ...
            : Row(
                children: [
                  ...
                  Expanded(
                    ...
                    child: Text(
                      '\$${item.subtotal.toStringAsFixed(2)}',
                      style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF9AC53B)),
                    ),
                  ),
                  IconButton(
                    onPressed: () => widget.onRemove(item.product.id),
        ...
        children: [
          ...
          Expanded(
            ...
            child: ElevatedButton.icon(
              onPressed: hasItems && !_isProcessing ? () async {
                setState(() => _isProcessing = true);
                await widget.onCheckout();
                if (mounted) {
                  setState(() => _isProcessing = false);
                  // Cerrar la pantalla del carrito después del checkout exitoso
