...
class _CatalogAdminScreenState extends State<CatalogAdminScreen> {
  ...
  bool get _isCategory => _selectedCatalog == 0;
  bool get _isProduct => _selectedCatalog == 1;
  bool get _isClient => _selectedCatalog == 2;
  bool get _isPaymentMethod => _selectedCatalog == 4;
  ...
  // ============================================================
  // ABRIR PANTALLA DE EDICIÓN (solo para catálogos que no sean formas de pago)
  // ============================================================

  Future<void> _openEditor([Map<String, dynamic>? item]) async {
    if (_isDisposed || !mounted) return;
    if (_isPaymentMethod) return; // No permitir edición para formas de pago
  ...
  // ============================================================
  // TOGGLE ACTIVO (para formas de pago)
  // ============================================================
  ...
  Future<void> _toggleStatus(Map<String, dynamic> item) async {
    ...
    try {
      await _db.updateCatalogItem(
        table: _currentTable,
        id: id,
        // No modificamos nombre, código ni tasa, solo activo
  ...
  // ============================================================
  // ELIMINAR (desactivar) para otros catálogos
  // ============================================================

  Future<void> _delete(Map<String, dynamic> item) async {
    if (_isDisposed || !mounted) return;
    if (_isPaymentMethod) return; // No se usa para formas de pago
  ...
  // ============================================================
  // TOGGLE SWITCH PARA FORMAS DE PAGO
  // ============================================================
            ...
            child: Center(
              child: Column(
                ...
                children: [
                  ...
                  // Solo mostramos el botón de agregar si NO es formas de pago
                  if (!_isPaymentMethod)
                    FilledButton.icon(
                      onPressed: () =>
                          _openEditor(),
  ...
  Widget _buildFAB(
    ...
    // Ocultar FAB para formas de pago
    if (_isPaymentMethod) {
