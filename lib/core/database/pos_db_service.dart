...
class PosDatabaseService {
  ...
  String _key(int companyId, int userId, DateTime date) =>
      '$companyId:$userId:${dateKey(date)}';
  ...
  Future<void> _indexes(Database db) async {
    await db.execute(
      'CREATE INDEX IF NOT EXISTS idx_day_sales_status ON sales(sync_status)',
    );
    await db.execute(
      'CREATE INDEX IF NOT EXISTS idx_day_sales_date ON sales(business_date)',
    );
    await db.execute(
      'CREATE INDEX IF NOT EXISTS idx_day_outbox_status ON sync_outbox(status)',
    );
    await db.execute(
      'CREATE INDEX IF NOT EXISTS idx_day_outbox_retry ON sync_outbox(next_retry_at)',
    );
  }
    ...
    await db.transaction((txn) async {
      ...
      for (final product in products) {
        ...
        await txn.insert('products', {
          ...
          'price': _toDouble(
            product['price'] ?? product['precio'] ?? product['precio_venta'],
          ),
          'stock': _toDouble(
            product['stock'] ?? product['existencia'] ?? product['cantidad'],
          ),
  ...
  )).query('products', where: 'deleted_at IS NULL', orderBy: 'name ASC');
    ...
    return db.transaction((txn) async {
      ...
      for (final payment in payments) {
        await txn.insert('sale_payments', {
          'sale_id': saleId,
          'method': payment['method'] ?? payment['forma_pago'],
  ...
  Future<List<Map<String, dynamic>>> getTodaySales({
    ...
    required DateTime businessDate,
      ...
      (await open(
        ...
        'sales',
        where: 'business_date = ?',
        whereArgs: [dateKey(businessDate)],
  ...
  Future<Map<String, dynamic>?> getSaleByUuid({
    required int companyId,
    required int userId,
    required DateTime businessDate,
    required String uuidLocal,
  ...
  Future<List<Map<String, dynamic>>> getSaleItems(Database db, int saleId) =>
      db.query('sale_items', where: 'sale_id = ?', whereArgs: [saleId]);
  Future<List<Map<String, dynamic>>> getSalePayments(Database db, int saleId) =>
      db.query('sale_payments', where: 'sale_id = ?', whereArgs: [saleId]);
  ...
  Future<void> markOutboxSynced({
    ...
    final data = serverResponse?['data'] is Map
        ? Map<String, dynamic>.from(serverResponse!['data'])
        : (serverResponse ?? const {});
    final serverId = _nullableInt(
      data['server_id'] ?? data['venta_id'] ?? data['id'],
    );
  ...
  Future<void> markOutboxFailed({
    ...
    final retry = DateTime.now().add(
      Duration(minutes: (attempts.clamp(1, 30)) * 2),
    );
    await db.transaction((txn) async {
      await txn.update(
        'sync_outbox',
